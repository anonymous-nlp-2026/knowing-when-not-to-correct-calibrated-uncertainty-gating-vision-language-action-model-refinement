...
def parse_args():
    ...
    p.add_argument("--mode", type=str, default="crm_perdof",
                   choices=["vla_only", "crm_scalar", "crm_perdof", "acr_vla", "crm_obs_only", "reconvla"])
...
def evaluate(args):
    ...
    # CRM (if needed)
    crm = None
    if args.mode in ("crm_scalar", "crm_perdof", "acr_vla", "crm_obs_only"):
        if args.checkpoint is None:
            raise ValueError("--checkpoint required for CRM modes")

        if args.mode == "crm_obs_only":
            unc_dim = 0
